...
HashMap是由一个包含了key和value的Node数组存储的。通过Hash定位数组下标
...
下面代码实质就是返回大于等于cap，并且是2的指数幂的最小值
...
删除制定key对应的节点，返回key对应的旧值删除操作与get查找操作过程是类似的，根据key找到对应的键值对，
然后进行删除。如果不存在hash冲突， 可以直接将table对应位置置为null，否则需要在链表或红黑树中删除节点。
...
散列表在设计的时候，地址应该是取模获得的，但是取模操作效率太低。当容量是2的指数幂时，hash & (cap - 1) 
等于 hash % cap。按位与能提高计算效率，容量设置为2的指数幂时，就可以用按位与来取模了。

### hash冲突为什么一开始不用红黑树

红黑树的维修成本很高，如果一开始就用红黑树解决冲突，效率会低很多。只有当表足够大，冲突节点足够多的情况下才使用红黑树。
